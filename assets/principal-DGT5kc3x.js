import{j as b}from"./vendor-gzd0YkcT.js";import{v as X}from"./wii-BArvJV_J.js";import"./main-Bf8DocAo.js";import"./main-CJcK61zC.js";const P=["¡Genial! 🎯 Las matemáticas son divertidas","¡Excelente pregunta! 🌟 Aquí va la respuesta","¡Me encanta resolver esto! 🚀 Vamos allá","¡Qué emoción! 💡 Resolvamos esto juntos","¡Perfecto! 🎨 Las mates son arte","¡Súper! ⭐ Te va a encantar esto"],A=["¡Boom! 💥 Ahí está la respuesta","¡Tachán! ✨ Resuelto","¡Voilà! 🎭 Problema solucionado","¡Eureka! 🎉 Lo logramos","¡Bingo! 🎯 Ahí lo tienes","¡Fantástico! 🌟 Resultado final"],x=o=>o[Math.floor(Math.random()*o.length)],K=o=>{const e=o.toLowerCase(),a=e.match(/(\d+)x\s*([+-])\s*(\d+)\s*=\s*(\d+)/);if(a){const[n,r,t,i,c]=a,d=parseInt(r),g=t==="+"?parseInt(i):-parseInt(i),h=parseInt(c)-g,y=h/d;return`${x(P)} 😊<br><br>📊 <strong>Resolución paso a paso:</strong><br><br><strong>Ecuación:</strong> ${r}x ${t} ${i} = ${c}<br><br><strong>Paso 1:</strong> Despejar x moviendo el ${Math.abs(g)} 🔄<br>${r}x = ${c} ${t==="+"?"-":"+"} ${Math.abs(g)}<br>${r}x = ${h}<br><br><strong>Paso 2:</strong> Dividir ambos lados entre ${r} ➗<br>x = ${h} ÷ ${r}<br><br>${x(A)} 🎊<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ x = ${y}</span><br><br><em>🔍 Verificación: ${r}(${y}) ${t} ${i} = ${d*y+g} = ${c} ✓</em> 😄`}if(/porcentaje|%/.test(e)){const n=e.match(/(\d+)%\s*de\s*(\d+)/);if(n){const[r,t,i]=n,c=parseInt(t)*parseInt(i)/100;return`${x(P)} 📈<br><br>📊 <strong>Cálculo de porcentaje:</strong><br><br>${t}% de ${i} = (${t} × ${i}) ÷ 100 🧮<br>= ${parseInt(t)*parseInt(i)} ÷ 100<br><br>${x(A)} 🎉<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ Resultado: ${c}</span> 😊`}}const s=e.match(/(\d+)\s*([+\-*/])\s*(\d+)/);if(s){const[n,r,t,i]=s,c=parseFloat(r),d=parseFloat(i);let g,v,h;switch(t){case"+":g=c+d,v="+",h="➕";break;case"-":g=c-d,v="-",h="➖";break;case"*":g=c*d,v="×",h="✖️";break;case"/":g=c/d,v="÷",h="➗";break}return`${x(P)} ${h}<br><br>🔢 <strong>Operación:</strong><br><br>${r} ${v} ${i} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${g}</span> ✅ 😄`}if(/área|perímetro|cuadrado|rectángulo|círculo/.test(e)){const n=e.match(/cuadrado.*?(\d+)/);if(n){const r=parseInt(n[1]);return`¡Me encantan las figuras geométricas! 📐😊<br><br><strong>Cuadrado de lado ${r}:</strong><br><br>• <strong>Área</strong> = lado² = ${r}² = <span style="color: #2196f3; font-weight: bold;">${r*r}</span> unidades² 🟦<br>• <strong>Perímetro</strong> = 4 × lado = 4 × ${r} = <span style="color: #2196f3; font-weight: bold;">${4*r}</span> unidades 📏<br><br>¡Facilísimo! 🎉😄`}}if(/raíz cuadrada|√/.test(e)){const n=e.match(/raíz cuadrada de (\d+)|√(\d+)/);if(n){const r=parseInt(n[1]||n[2]),t=Math.sqrt(r),i=Number.isInteger(t);return`${x(P)} 🌱<br><br>🔢 <strong>Raíz cuadrada:</strong><br><br>√${r} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${t.toFixed(2)}</span> ${i?"✅ ¡Exacto!":"<em>(aproximado)</em>"} 😊`}}if(/elevado|potencia|\^/.test(e)){const n=e.match(/(\d+)\s*(?:elevado a|potencia|\^)\s*(\d+)/);if(n){const[r,t,i]=n,c=Math.pow(parseInt(t),parseInt(i));return`¡Las potencias son poderosas! 💪⚡<br><br>🔢 <strong>Potencia:</strong><br><br>${t}<sup>${i}</sup> = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${c}</span> ✅<br><br>${x(A)} 🎊😄`}}if(/tabla de multiplicar|tabla del/i.test(e)){const n=e.match(/tabla del? (\d+)/i);if(n){const r=parseInt(n[1]);let t=`¡La tabla del ${r}! 📚 ¡Vamos a aprenderla! 😊<br><br>`;t+=`🔢 <strong>Tabla de multiplicar del ${r}:</strong><br><br>`;for(let i=1;i<=10;i++)t+=`${r} × ${i} = <strong style="color: #2196f3;">${r*i}</strong> ${i===5?"🌟":""}<br>`;return t+="<br>¡Practica y serás un experto! 🚀😄",t}}if(/primo|primos/.test(e)){const n=e.match(/(\d+)\s+es primo/);if(n){const r=parseInt(n[1]),i=(c=>{if(c<=1)return!1;if(c<=3)return!0;if(c%2===0||c%3===0)return!1;for(let d=5;d*d<=c;d+=6)if(c%d===0||c%(d+2)===0)return!1;return!0})(r);return`🔍 Analizando si ${r} es primo...<br><br>${i?`¡Sí! 🎉 <strong style="color: #4caf50;">${r} ES un número primo</strong> 😊<br><br>Solo es divisible por 1 y por sí mismo 🌟`:`No 😔 <strong style="color: #f44336;">${r} NO es primo</strong><br><br>Es divisible por otros números además de 1 y ${r} 🔢`}`}}return null},Z={formulas:{cuadrado:{area:"lado²",perimetro:"4 × lado"},rectangulo:{area:"base × altura",perimetro:"2(base + altura)"},circulo:{area:"π × radio²",perimetro:"2π × radio"},triangulo:{area:"(base × altura) / 2"}},conceptos:{ecuacion:"Igualdad matemática con incógnitas",fraccion:"Parte de un todo (numerador/denominador)",porcentaje:"Fracción de 100 (x/100)",primo:"Número solo divisible por 1 y por sí mismo"}},ee=Object.freeze(Object.defineProperty({__proto__:null,generate:K,knowledge:Z},Symbol.toStringTag,{value:"Module"}));let q=[];const O=20,Q=o=>{if(typeof o=="string")return o;if(o==null)return"";try{return String(o)}catch{return""}},T=o=>{const e={...o,content:Q(o.content),timestamp:Date.now()};q.push(e),q.length>O&&(q=q.slice(-O))},J=(o=5)=>q.slice(-o),F=()=>q.length>0,ae=o=>q.filter(e=>e.content.toLowerCase().includes(o.toLowerCase())),oe=()=>{q=[]},se=o=>{const e=Q(o).toLowerCase(),a=J(5);if(/anterior|antes|dijiste|mencionaste|lo que|eso|aquello/.test(e)){const s=[...a].reverse().find(n=>n.role==="assistant");if(s)return`🧠 **Recordando la conversación...**
...
Estoy aquí si necesitas hablar más. 💚`}return/solo|soledad|nadie|aislado/.test(e)?`${M.soledad[Math.floor(Math.random()*M.soledad.length)]}
...
Recuerda: **La soledad es temporal. Las conexiones empiezan con un paso.** 🫂`:/motivación|motivado|ganas|ánimo|energía/.test(e)?`${M.motivacion[Math.floor(Math.random()*M.motivacion.length)]}
...
`+M.mejora.slice(0,3).map((s,n)=>`${n+1}. ${s}`).join(`
...
¿Sobre qué tema específico quieres aprender?`}if(/dónde está|dónde queda|país|ciudad|montaña|río/i.test(e)){for(const[a,s]of Object.entries(N.geografia))if(e.includes(a))return`🌍 **Geografía:**
...
🕊️ *- Mediawii*`:null},qe=Object.freeze(Object.defineProperty({__proto__:null,generate:ye},Symbol.toStringTag,{value:"Module"})),ze=o=>{const e=o.toLowerCase();if(/javascript|js|función|arrow function|const|let|var/i.test(e)){if(/función|function/i.test(e))return"💻 **Funciones en JavaScript:**\n\n```javascript\n// Función tradicional\nfunction saludar(nombre) {\n  return `Hola ${nombre}`;\n}\n\n// Arrow function (moderna)\nconst saludar = (nombre) => `Hola ${nombre}`;\n```\n\n💡 Las arrow functions son más concisas y no tienen su propio `this`.";if(/array|map|filter|reduce/i.test(e))return`💻 **Métodos de Array en JavaScript:**
...
por siempre jamás. Amén. 🙏`},xe=o=>{const e=o.toLowerCase();if(/verso|versículo|cita|biblia|pasaje|escritura/i.test(e)){let a="esperanza";/esperanza|futuro|planes/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor|valiente/i.test(e)?a="fortaleza":/paz|tranquil|calm|ansiedad|preocup/i.test(e)?a="paz":/amor|amo|quiero|amado/i.test(e)?a="amor":/fe|creer|cree|confiar/i.test(e)?a="fe":/sabio|sabiduría|consejo|decisión/i.test(e)?a="sabiduria":/perdón|perdonar|arrepent/i.test(e)?a="perdon":/protección|proteger|cuidar|segur/i.test(e)?a="proteccion":/provisión|proveer|necesidad|dinero|trabajo/i.test(e)&&(a="provision");const s=_[a],n=s[Math.floor(Math.random()*s.length)];return`📖 **${n.cita}**
...
🙏 Que este verso ilumine tu corazón hoy. Amén.`}if(/oración|ora|recemos|reza/i.test(e))return/padre nuestro/i.test(e)?C.padrenuestro:/mañana|despertar|día/i.test(e)?C.manana:/noche|dormir|descansar/i.test(e)?C.noche:/difícil|problema|ayuda|crisis/i.test(e)?C.dificil:/gracias|agradec/i.test(e)?C.agradecimiento:`🙏 **Oremos juntos:**
...
¿Hay algo específico por lo que quieras que oremos?`;if(/consejo|ayuda|qué hago|no sé qué hacer/i.test(e)){const a=_.sabiduria[Math.floor(Math.random()*_.sabiduria.length)];return`💡 **Consejo Bíblico:**
...
🙏 Dios no teme tus preguntas. Acércate a Él con sinceridad.`;if(/dios me ama|me ama dios|por qué dios/i.test(e)){const a=_.amor[Math.floor(Math.random()*_.amor.length)];return`💚 **Dios te ama más de lo que imaginas:**
...
`,we=()=>{const o=b("#miiaMessages"),e=b("#miiaInput"),a=b("#miiaSend");let s=!1,n=0,r=!0;const t=120,i=l=>{if(typeof l!="string")return"";let m=l.replace(/<script[\s\S]*?>[\s\S]*?<\/script>/gi,"").replace(/<style[\s\S]*?>[\s\S]*?<\/style>/gi,"").replace(/\son\w+="[^"]*"/gi,"").replace(/\son\w+='[^']*'/gi,"").replace(/javascript:/gi,"").trim();const p=/(br|strong|b|em|i|u|code|pre|p|ul|ol|li|blockquote)/i;return m=m.replace(/<([^>]+)>/g,(f,z)=>{const S=z.split(" ")[0].replace(/\//g,"");return p.test(S)?`<${z}>`:""}),m};o.on("scroll",function(){r=this.scrollHeight-this.scrollTop-this.clientHeight<=t}),e.on("input",function(){this.style.height="auto";const l=Math.min(this.scrollHeight,120);this.style.height=l+"px";const m=b(this).val().trim().length>0;a.prop("disabled",!m),a.toggleClass("active",m)});const c=async()=>{const l=e.val().trim();if(!(!l||s)){b(".miia_empty").fadeOut(300,function(){b(this).remove()}),d(l,"user",!0),e.val("").css("height","auto").trigger("input"),s=!0,g();try{const m=await De(l);console.debug("[ChatWiil] raw brain response:",m);const p=i(m);if(console.debug("[ChatWiil] sanitized response:",p),v(),typeof p!="string"||p.length===0)throw new Error("Respuesta no válida del cerebro");h(p,"ai",()=>{s=!1})}catch(m){console.error("❌ Error procesando mensaje:",m),v(),d("😔 Disculpa, tuve un problema procesando tu mensaje. Por favor, intenta de nuevo. 💚","ai",!0),s=!1}}};e.on("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),c())}),a.on("click",c),b(document).on("click",".suggestion_card",function(){const l=b(this).data("prompt");e.val(l).css("height","auto").trigger("input"),e.focus()});function d(l,m,p=!1){const f=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),z=m==="user"?'<i class="fas fa-user-circle"></i>':'<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',S=m==="user"?"Tú":"ChatWiil",$=m==="user"?l:i(l),L=m==="user"?$.replace(/\n/g,"<br>"):$,E=`
      <div class="miia_message ${m}" data-time="${f}">
        <div class="message_avatar">${z}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">${S}</span>
            <span class="message_time">${f}</span>
          </div>
          <div class="message_text">${L}</div>
        </div>
      </div>
    `;o.append(E),y(p)}function g(){v(),o.append(`
    ...
    `),y()}function v(){b(".miia_message.typing").remove()}function h(l,D,p){const f=i(l);if(console.debug("[ChatWiil] typewriter safeText:",f),typeof f!="string"||f.length===0){console.error("❌ typeWriterEffect recibió texto inválido:",l),d("🤔 Lo siento, hubo un error al generar la respuesta.","ai",!0),p&&p();return}const z=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),S='<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',$=`typewriter_${Date.now()}_${++n}`,L=`
      <div class="miia_message ${D}" data-time="${z}">
        <div class="message_avatar">${S}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">ChatWiil</span>
            <span class="message_time">${z}</span>
          </div>
          <div class="message_text" id="${$}"></div>
        </div>
      </div>
    `;o.append(L);const E=b(`#${$}`);if(E.length===0){console.error("❌ No se encontró el elemento typewriter con ID:",$),p&&p();return}let j=0;const U=15;function D(){if(j<f.length){if(f[j]==="<"){const w=f.indexOf(">",j);if(w!==-1){const Y=f.substring(j,w+1);E.append(Y),j=w+1,y(),setTimeout(D,0);return}}E.append(f.charAt(j)),j++,y(),setTimeout(D,U)}else E.removeAttr("id"),p&&p()}y(),D()}function y(l=!1){!l&&!r||o.stop(!0).animate({scrollTop:o[0].scrollHeight},200)}console.log(`✅ ChatWiil IA ${X} iniciado con 🧠 Brain System + 🎭 Personalidad`)},Ae=()=>{b("#miiaInput, #miiaSend").off(),b(document).off("click",".suggestion_card"),console.log("🧹 ChatWiil IA limpiado")};export{Ae as cleanup,we as init,Le as render};
