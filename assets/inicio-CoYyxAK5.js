import{j as g}from"./vendor-gzd0YkcT.js";import{v as G}from"./wii-Dxlft3j-.js";import"./main-BT0v1kez.js";import"./main-CbMACaX3.js";const E=["¡Genial! 🎯 Las matemáticas son divertidas","¡Excelente pregunta! 🌟 Aquí va la respuesta","¡Me encanta resolver esto! 🚀 Vamos allá","¡Qué emoción! 💡 Resolvamos esto juntos","¡Perfecto! 🎨 Las mates son arte","¡Súper! ⭐ Te va a encantar esto"],C=["¡Boom! 💥 Ahí está la respuesta","¡Tachán! ✨ Resuelto","¡Voilà! 🎭 Problema solucionado","¡Eureka! 🎉 Lo logramos","¡Bingo! 🎯 Ahí lo tienes","¡Fantástico! 🌟 Resultado final"],h=o=>o[Math.floor(Math.random()*o.length)],B=o=>{const e=o.toLowerCase(),a=e.match(/(\d+)x\s*([+-])\s*(\d+)\s*=\s*(\d+)/);if(a){const[s,r,i,t,l]=a,m=parseInt(r),p=i==="+"?parseInt(t):-parseInt(t),u=parseInt(l)-p,f=u/m;return`${h(E)} 😊<br><br>📊 <strong>Resolución paso a paso:</strong><br><br><strong>Ecuación:</strong> ${r}x ${i} ${t} = ${l}<br><br><strong>Paso 1:</strong> Despejar x moviendo el ${Math.abs(p)} 🔄<br>${r}x = ${l} ${i==="+"?"-":"+"} ${Math.abs(p)}<br>${r}x = ${u}<br><br><strong>Paso 2:</strong> Dividir ambos lados entre ${r} ➗<br>x = ${u} ÷ ${r}<br><br>${h(C)} 🎊<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ x = ${f}</span><br><br><em>🔍 Verificación: ${r}(${f}) ${i} ${t} = ${m*f+p} = ${l} ✓</em> 😄`}if(/porcentaje|%/.test(e)){const s=e.match(/(\d+)%\s*de\s*(\d+)/);if(s){const[r,i,t]=s,l=parseInt(i)*parseInt(t)/100;return`${h(E)} 📈<br><br>📊 <strong>Cálculo de porcentaje:</strong><br><br>${i}% de ${t} = (${i} × ${t}) ÷ 100 🧮<br>= ${parseInt(i)*parseInt(t)} ÷ 100<br><br>${h(C)} 🎉<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ Resultado: ${l}</span> 😊`}}const n=e.match(/(\d+)\s*([+\-*/])\s*(\d+)/);if(n){const[s,r,i,t]=n,l=parseFloat(r),m=parseFloat(t);let p,c,u;switch(i){case"+":p=l+m,c="+",u="➕";break;case"-":p=l-m,c="-",u="➖";break;case"*":p=l*m,c="×",u="✖️";break;case"/":p=l/m,c="÷",u="➗";break}return`${h(E)} ${u}<br><br>🔢 <strong>Operación:</strong><br><br>${r} ${c} ${t} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${p}</span> ✅ 😄`}if(/área|perímetro|cuadrado|rectángulo|círculo/.test(e)){const s=e.match(/cuadrado.*?(\d+)/);if(s){const r=parseInt(s[1]);return`¡Me encantan las figuras geométricas! 📐😊<br><br><strong>Cuadrado de lado ${r}:</strong><br><br>• <strong>Área</strong> = lado² = ${r}² = <span style="color: #2196f3; font-weight: bold;">${r*r}</span> unidades² 🟦<br>• <strong>Perímetro</strong> = 4 × lado = 4 × ${r} = <span style="color: #2196f3; font-weight: bold;">${4*r}</span> unidades 📏<br><br>¡Facilísimo! 🎉😄`}}if(/raíz cuadrada|√/.test(e)){const s=e.match(/raíz cuadrada de (\d+)|√(\d+)/);if(s){const r=parseInt(s[1]||s[2]),i=Math.sqrt(r),t=Number.isInteger(i);return`${h(E)} 🌱<br><br>🔢 <strong>Raíz cuadrada:</strong><br><br>√${r} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${i.toFixed(2)}</span> ${t?"✅ ¡Exacto!":"<em>(aproximado)</em>"} 😊`}}if(/elevado|potencia|\^/.test(e)){const s=e.match(/(\d+)\s*(?:elevado a|potencia|\^)\s*(\d+)/);if(s){const[r,i,t]=s,l=Math.pow(parseInt(i),parseInt(t));return`¡Las potencias son poderosas! 💪⚡<br><br>🔢 <strong>Potencia:</strong><br><br>${i}<sup>${t}</sup> = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${l}</span> ✅<br><br>${h(C)} 🎊😄`}}if(/tabla de multiplicar|tabla del/i.test(e)){const s=e.match(/tabla del? (\d+)/i);if(s){const r=parseInt(s[1]);let i=`¡La tabla del ${r}! 📚 ¡Vamos a aprenderla! 😊<br><br>`;i+=`🔢 <strong>Tabla de multiplicar del ${r}:</strong><br><br>`;for(let t=1;t<=10;t++)i+=`${r} × ${t} = <strong style="color: #2196f3;">${r*t}</strong> ${t===5?"🌟":""}<br>`;return i+="<br>¡Practica y serás un experto! 🚀😄",i}}if(/primo|primos/.test(e)){const s=e.match(/(\d+)\s+es primo/);if(s){const r=parseInt(s[1]),t=(l=>{if(l<=1)return!1;if(l<=3)return!0;if(l%2===0||l%3===0)return!1;for(let m=5;m*m<=l;m+=6)if(l%m===0||l%(m+2)===0)return!1;return!0})(r);return`🔍 Analizando si ${r} es primo...<br><br>${t?`¡Sí! 🎉 <strong style="color: #4caf50;">${r} ES un número primo</strong> 😊<br><br>Solo es divisible por 1 y por sí mismo 🌟`:`No 😔 <strong style="color: #f44336;">${r} NO es primo</strong><br><br>Es divisible por otros números además de 1 y ${r} 🔢`}`}}return null},V={formulas:{cuadrado:{area:"lado²",perimetro:"4 × lado"},rectangulo:{area:"base × altura",perimetro:"2(base + altura)"},circulo:{area:"π × radio²",perimetro:"2π × radio"},triangulo:{area:"(base × altura) / 2"}},conceptos:{ecuacion:"Igualdad matemática con incógnitas",fraccion:"Parte de un todo (numerador/denominador)",porcentaje:"Fracción de 100 (x/100)",primo:"Número solo divisible por 1 y por sí mismo"}},W=Object.freeze(Object.defineProperty({__proto__:null,generate:B,knowledge:V},Symbol.toStringTag,{value:"Module"}));let b=[];const w=20,D=o=>{b.push({...o,timestamp:Date.now()}),b.length>w&&(b=b.slice(-w))},O=(o=5)=>b.slice(-o),I=()=>b.length>0,U=o=>b.filter(e=>e.content.toLowerCase().includes(o.toLowerCase())),Y=()=>{b=[]},X=o=>{const e=o.toLowerCase(),a=O(5);if(/anterior|antes|dijiste|mencionaste|lo que|eso|aquello/.test(e)){const n=[...a].reverse().find(s=>s.role==="assistant");if(n)return`🧠 **Recordando la conversación...**
...
Háblame con confianza, te escucho. 🕊️`:null},ne=Object.freeze(Object.defineProperty({__proto__:null,generate:se},Symbol.toStringTag,{value:"Module"})),N={desconocido:["🤔 Hmm, no estoy seguro de cómo responder a eso. ¿Podrías reformular tu pregunta?","💭 Interesante pregunta. ¿Podrías darme más detalles para ayudarte mejor?","🕊️ No tengo una respuesta clara para eso, pero intentaré ayudarte. ¿Puedes ser más específico?"]},re={esperanza:['📖 Jeremías 29:11: "Yo sé los planes que tengo para ustedes, planes de bienestar y no de calamidad, a fin de darles un futuro y una esperanza."','📖 Romanos 15:13: "Que el Dios de la esperanza los llene de toda alegría y paz en la fe, para que rebosen de esperanza por el poder del Espíritu Santo."','📖 Salmo 33:18: "Los ojos del Señor están sobre los que le temen, sobre los que esperan en su gran amor."'],fortaleza:['📖 Isaías 41:10: "No temas, porque yo estoy contigo; no desmayes, porque yo soy tu Dios que te fortalezco."','📖 Filipenses 4:13: "Todo lo puedo en Cristo que me fortalece."','📖 Josué 1:9: "Sé fuerte y valiente. No temas ni desmayes, porque el Señor tu Dios estará contigo dondequiera que vayas."'],paz:['📖 Juan 14:27: "La paz les dejo, mi paz les doy. No se la doy como la da el mundo. No se turbe su corazón ni tenga miedo."','📖 Filipenses 4:6-7: "No se preocupen por nada; en cambio, oren por todo. Díganle a Dios lo que necesitan."','📖 Isaías 26:3: "Tú guardarás en completa paz a aquel cuyo pensamiento en ti persevera."'],amor:['📖 1 Juan 4:8: "El que no ama no ha conocido a Dios, porque Dios es amor."','📖 Juan 3:16: "Porque de tal manera amó Dios al mundo, que ha dado a su Hijo unigénito."','📖 Romanos 8:38-39: "Nada nos podrá separar del amor de Dios que es en Cristo Jesús."']},L=o=>{const e=o.toLowerCase();if(/verso|versículo|biblia|pasaje|cita bíblica|escritura bíblica/i.test(e)){let a="esperanza";/esperanza|futuro|fe/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor/i.test(e)?a="fortaleza":/paz|tranquil|calm/i.test(e)?a="paz":/amor|amo|quiero/i.test(e)&&(a="amor");const n=re[a];return n[Math.floor(Math.random()*n.length)]+"<br><br>💚 Que este verso ilumine tu día."}return/padre nuestro/i.test(e)?"🙏 <strong>El Padre Nuestro:</strong><br><br>Padre nuestro que estás en el cielo,<br>santificado sea tu nombre.<br>Venga tu reino,<br>hágase tu voluntad en la tierra como en el cielo.<br>Danos hoy nuestro pan de cada día,<br>perdona nuestras ofensas,<br>como también nosotros perdonamos a los que nos ofenden.<br>No nos dejes caer en tentación,<br>y líbranos del mal.<br>Amén. 🙏":N.desconocido[Math.floor(Math.random()*N.desconocido.length)]},te=(o,e)=>o.length===0?L(e):o.length===1?o[0].text:o.map(n=>`<strong>[${n.module}]</strong><br>${n.text}`).join("<br><br>---<br><br>"),ie=Object.freeze(Object.defineProperty({__proto__:null,generate:L,merge:te},Symbol.toStringTag,{value:"Module"})),ce=o=>{const e=o.toLowerCase();if(/por qué|porque|razón|motivo|explicar|explica/i.test(e)){if(/por qué exist|por qué estoy aquí|sentido de la vida/i.test(e))return`🤔 **Gran pregunta filosófica.**
...
¿Sobre qué tema específico quieres aprender?`}if(/dónde está|dónde queda|país|ciudad|montaña|río/i.test(e)){for(const[a,n]of Object.entries(T.geografia))if(e.includes(a))return`🌍 **Geografía:**
...
🕊️ *- Mediawii*`:null},fe=Object.freeze(Object.defineProperty({__proto__:null,generate:pe},Symbol.toStringTag,{value:"Module"})),ge=o=>{const e=o.toLowerCase();if(/javascript|js|función|arrow function|const|let|var/i.test(e)){if(/función|function/i.test(e))return"💻 **Funciones en JavaScript:**\n\n```javascript\n// Función tradicional\nfunction saludar(nombre) {\n  return `Hola ${nombre}`;\n}\n\n// Arrow function (moderna)\nconst saludar = (nombre) => `Hola ${nombre}`;\n```\n\n💡 Las arrow functions son más concisas y no tienen su propio `this`.";if(/array|map|filter|reduce/i.test(e))return`💻 **Métodos de Array en JavaScript:**
...
por siempre jamás. Amén. 🙏`},ve=o=>{const e=o.toLowerCase();if(/verso|versículo|cita|biblia|pasaje|escritura/i.test(e)){let a="esperanza";/esperanza|futuro|planes/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor|valiente/i.test(e)?a="fortaleza":/paz|tranquil|calm|ansiedad|preocup/i.test(e)?a="paz":/amor|amo|quiero|amado/i.test(e)?a="amor":/fe|creer|cree|confiar/i.test(e)?a="fe":/sabio|sabiduría|consejo|decisión/i.test(e)?a="sabiduria":/perdón|perdonar|arrepent/i.test(e)?a="perdon":/protección|proteger|cuidar|segur/i.test(e)?a="proteccion":/provisión|proveer|necesidad|dinero|trabajo/i.test(e)&&(a="provision");const n=j[a],s=n[Math.floor(Math.random()*n.length)];return`📖 **${s.cita}**
...
🙏 Que este verso ilumine tu corazón hoy. Amén.`}if(/oración|ora|recemos|reza/i.test(e))return/padre nuestro/i.test(e)?$.padrenuestro:/mañana|despertar|día/i.test(e)?$.manana:/noche|dormir|descansar/i.test(e)?$.noche:/difícil|problema|ayuda|crisis/i.test(e)?$.dificil:/gracias|agradec/i.test(e)?$.agradecimiento:`🙏 **Oremos juntos:**
...
`,Se=()=>{const o=g("#miiaMessages"),e=g("#miiaInput"),a=g("#miiaSend");let n=!1,s=0;e.on("input",function(){this.style.height="auto";const c=Math.min(this.scrollHeight,120);this.style.height=c+"px";const u=g(this).val().trim().length>0;a.prop("disabled",!u),a.toggleClass("active",u)});const r=async()=>{const c=e.val().trim();if(!(!c||n)){g(".miia_empty").fadeOut(300,function(){g(this).remove()}),i(c,"user"),e.val("").css("height","auto").trigger("input"),n=!0,t();try{const u=await je(c);if(l(),typeof u!="string")throw new Error("Respuesta no válida del cerebro");m(u,"ai",()=>{n=!1})}catch(u){console.error("❌ Error procesando mensaje:",u),l(),i("😔 Disculpa, tuve un problema procesando tu mensaje. Por favor, intenta de nuevo. 💚","ai"),n=!1}}};e.on("keydown",c=>{c.key==="Enter"&&!c.shiftKey&&(c.preventDefault(),r())}),a.on("click",r),g(document).on("click",".suggestion_card",function(){const c=g(this).data("prompt");e.val(c).css("height","auto").trigger("input"),e.focus()});function i(c,u){const f=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),M=u==="user"?'<i class="fas fa-user-circle"></i>':'<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',S=u==="user"?"Tú":"ChatWiil",q=u==="user"?c.replace(/\n/g,"<br>"):c,_=`
      <div class="miia_message ${u}" data-time="${f}">
        <div class="message_avatar">${M}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">${S}</span>
            <span class="message_time">${f}</span>
          </div>
          <div class="message_text">${q}</div>
        </div>
      </div>
    `;o.append(_),p()}function t(){l(),o.append(`
    ...
    `),p()}function l(){g(".miia_message.typing").remove()}function m(c,z,f){if(typeof c!="string"||c.length===0){console.error("❌ typeWriterEffect recibió texto inválido:",c),i("🤔 Lo siento, hubo un error al generar la respuesta.","ai"),f&&f();return}const M=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),S='<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',q=`typewriter_${Date.now()}_${++s}`,_=`
      <div class="miia_message ${z}" data-time="${M}">
        ...
        <div class="message_content">
          ...
          <div class="message_text" id="${q}"></div>
        </div>
      </div>
    `;o.append(_);const x=g(`#${q}`);if(x.length===0){console.error("❌ No se encontró el elemento typewriter con ID:",q),f&&f();return}let v=0;const F=15;function z(){if(v<c.length){if(c[v]==="<"){const P=c.indexOf(">",v);if(P!==-1){const k=c.substring(v,P+1);x.append(k),v=P+1,p(),setTimeout(z,0);return}}x.append(c.charAt(v)),v++,p(),setTimeout(z,F)}else x.removeAttr("id"),f&&f()}z()}function p(){o.animate({scrollTop:o[0].scrollHeight},300)}console.log(`✅ ChatWiil IA ${G} iniciado con 🧠 Brain System + 🎭 Personalidad`)},_e=()=>{g("#miiaInput, #miiaSend").off(),g(document).off("click",".suggestion_card"),console.log("🧹 ChatWiil IA limpiado")};export{_e as cleanup,Se as init,De as render};
