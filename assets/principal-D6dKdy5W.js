import{j as g}from"./vendor-gzd0YkcT.js";import{v as U}from"./wii-DxFiay5l.js";import"./main-B4sVu9Tq.js";import"./main-bfDW3XZl.js";const P=["¡Genial! 🎯 Las matemáticas son divertidas","¡Excelente pregunta! 🌟 Aquí va la respuesta","¡Me encanta resolver esto! 🚀 Vamos allá","¡Qué emoción! 💡 Resolvamos esto juntos","¡Perfecto! 🎨 Las mates son arte","¡Súper! ⭐ Te va a encantar esto"],A=["¡Boom! 💥 Ahí está la respuesta","¡Tachán! ✨ Resuelto","¡Voilà! 🎭 Problema solucionado","¡Eureka! 🎉 Lo logramos","¡Bingo! 🎯 Ahí lo tienes","¡Fantástico! 🌟 Resultado final"],j=o=>o[Math.floor(Math.random()*o.length)],Y=o=>{const e=o.toLowerCase(),a=e.match(/(\d+)x\s*([+-])\s*(\d+)\s*=\s*(\d+)/);if(a){const[s,r,i,t,c]=a,d=parseInt(r),m=i==="+"?parseInt(t):-parseInt(t),b=parseInt(c)-m,y=b/d;return`${j(P)} 😊<br><br>📊 <strong>Resolución paso a paso:</strong><br><br><strong>Ecuación:</strong> ${r}x ${i} ${t} = ${c}<br><br><strong>Paso 1:</strong> Despejar x moviendo el ${Math.abs(m)} 🔄<br>${r}x = ${c} ${i==="+"?"-":"+"} ${Math.abs(m)}<br>${r}x = ${b}<br><br><strong>Paso 2:</strong> Dividir ambos lados entre ${r} ➗<br>x = ${b} ÷ ${r}<br><br>${j(A)} 🎊<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ x = ${y}</span><br><br><em>🔍 Verificación: ${r}(${y}) ${i} ${t} = ${d*y+m} = ${c} ✓</em> 😄`}if(/porcentaje|%/.test(e)){const s=e.match(/(\d+)%\s*de\s*(\d+)/);if(s){const[r,i,t]=s,c=parseInt(i)*parseInt(t)/100;return`${j(P)} 📈<br><br>📊 <strong>Cálculo de porcentaje:</strong><br><br>${i}% de ${t} = (${i} × ${t}) ÷ 100 🧮<br>= ${parseInt(i)*parseInt(t)} ÷ 100<br><br>${j(A)} 🎉<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ Resultado: ${c}</span> 😊`}}const n=e.match(/(\d+)\s*([+\-*/])\s*(\d+)/);if(n){const[s,r,i,t]=n,c=parseFloat(r),d=parseFloat(t);let m,v,b;switch(i){case"+":m=c+d,v="+",b="➕";break;case"-":m=c-d,v="-",b="➖";break;case"*":m=c*d,v="×",b="✖️";break;case"/":m=c/d,v="÷",b="➗";break}return`${j(P)} ${b}<br><br>🔢 <strong>Operación:</strong><br><br>${r} ${v} ${t} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${m}</span> ✅ 😄`}if(/área|perímetro|cuadrado|rectángulo|círculo/.test(e)){const s=e.match(/cuadrado.*?(\d+)/);if(s){const r=parseInt(s[1]);return`¡Me encantan las figuras geométricas! 📐😊<br><br><strong>Cuadrado de lado ${r}:</strong><br><br>• <strong>Área</strong> = lado² = ${r}² = <span style="color: #2196f3; font-weight: bold;">${r*r}</span> unidades² 🟦<br>• <strong>Perímetro</strong> = 4 × lado = 4 × ${r} = <span style="color: #2196f3; font-weight: bold;">${4*r}</span> unidades 📏<br><br>¡Facilísimo! 🎉😄`}}if(/raíz cuadrada|√/.test(e)){const s=e.match(/raíz cuadrada de (\d+)|√(\d+)/);if(s){const r=parseInt(s[1]||s[2]),i=Math.sqrt(r),t=Number.isInteger(i);return`${j(P)} 🌱<br><br>🔢 <strong>Raíz cuadrada:</strong><br><br>√${r} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${i.toFixed(2)}</span> ${t?"✅ ¡Exacto!":"<em>(aproximado)</em>"} 😊`}}if(/elevado|potencia|\^/.test(e)){const s=e.match(/(\d+)\s*(?:elevado a|potencia|\^)\s*(\d+)/);if(s){const[r,i,t]=s,c=Math.pow(parseInt(i),parseInt(t));return`¡Las potencias son poderosas! 💪⚡<br><br>🔢 <strong>Potencia:</strong><br><br>${i}<sup>${t}</sup> = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${c}</span> ✅<br><br>${j(A)} 🎊😄`}}if(/tabla de multiplicar|tabla del/i.test(e)){const s=e.match(/tabla del? (\d+)/i);if(s){const r=parseInt(s[1]);let i=`¡La tabla del ${r}! 📚 ¡Vamos a aprenderla! 😊<br><br>`;i+=`🔢 <strong>Tabla de multiplicar del ${r}:</strong><br><br>`;for(let t=1;t<=10;t++)i+=`${r} × ${t} = <strong style="color: #2196f3;">${r*t}</strong> ${t===5?"🌟":""}<br>`;return i+="<br>¡Practica y serás un experto! 🚀😄",i}}if(/primo|primos/.test(e)){const s=e.match(/(\d+)\s+es primo/);if(s){const r=parseInt(s[1]),t=(c=>{if(c<=1)return!1;if(c<=3)return!0;if(c%2===0||c%3===0)return!1;for(let d=5;d*d<=c;d+=6)if(c%d===0||c%(d+2)===0)return!1;return!0})(r);return`🔍 Analizando si ${r} es primo...<br><br>${t?`¡Sí! 🎉 <strong style="color: #4caf50;">${r} ES un número primo</strong> 😊<br><br>Solo es divisible por 1 y por sí mismo 🌟`:`No 😔 <strong style="color: #f44336;">${r} NO es primo</strong><br><br>Es divisible por otros números además de 1 y ${r} 🔢`}`}}return null},X={formulas:{cuadrado:{area:"lado²",perimetro:"4 × lado"},rectangulo:{area:"base × altura",perimetro:"2(base + altura)"},circulo:{area:"π × radio²",perimetro:"2π × radio"},triangulo:{area:"(base × altura) / 2"}},conceptos:{ecuacion:"Igualdad matemática con incógnitas",fraccion:"Parte de un todo (numerador/denominador)",porcentaje:"Fracción de 100 (x/100)",primo:"Número solo divisible por 1 y por sí mismo"}},K=Object.freeze(Object.defineProperty({__proto__:null,generate:Y,knowledge:X},Symbol.toStringTag,{value:"Module"}));let q=[];const O=20,C=o=>{q.push({...o,timestamp:Date.now()}),q.length>O&&(q=q.slice(-O))},Q=(o=5)=>q.slice(-o),J=()=>q.length>0,Z=o=>q.filter(e=>e.content.toLowerCase().includes(o.toLowerCase())),ee=()=>{q=[]},ae=o=>{const e=o.toLowerCase(),a=Q(5);if(/anterior|antes|dijiste|mencionaste|lo que|eso|aquello/.test(e)){const n=[...a].reverse().find(s=>s.role==="assistant");if(n)return`🧠 **Recordando la conversación...**
...
Estoy aquí si necesitas hablar más. 💚`}return/solo|soledad|nadie|aislado/.test(e)?`${x.soledad[Math.floor(Math.random()*x.soledad.length)]}
...
Recuerda: **La soledad es temporal. Las conexiones empiezan con un paso.** 🫂`:/motivación|motivado|ganas|ánimo|energía/.test(e)?`${x.motivacion[Math.floor(Math.random()*x.motivacion.length)]}
...
`+x.mejora.slice(0,3).map((n,s)=>`${s+1}. ${n}`).join(`
...
Háblame con confianza, te escucho. 🕊️`:null},ce=Object.freeze(Object.defineProperty({__proto__:null,generate:ie},Symbol.toStringTag,{value:"Module"})),R={desconocido:["🤔 Hmm, no estoy seguro de cómo responder a eso. ¿Podrías reformular tu pregunta?","💭 Interesante pregunta. ¿Podrías darme más detalles para ayudarte mejor?","🕊️ No tengo una respuesta clara para eso, pero intentaré ayudarte. ¿Puedes ser más específico?"]},le={esperanza:['📖 Jeremías 29:11: "Yo sé los planes que tengo para ustedes, planes de bienestar y no de calamidad, a fin de darles un futuro y una esperanza."','📖 Romanos 15:13: "Que el Dios de la esperanza los llene de toda alegría y paz en la fe, para que rebosen de esperanza por el poder del Espíritu Santo."','📖 Salmo 33:18: "Los ojos del Señor están sobre los que le temen, sobre los que esperan en su gran amor."'],fortaleza:['📖 Isaías 41:10: "No temas, porque yo estoy contigo; no desmayes, porque yo soy tu Dios que te fortalezco."','📖 Filipenses 4:13: "Todo lo puedo en Cristo que me fortalece."','📖 Josué 1:9: "Sé fuerte y valiente. No temas ni desmayes, porque el Señor tu Dios estará contigo dondequiera que vayas."'],paz:['📖 Juan 14:27: "La paz les dejo, mi paz les doy. No se la doy como la da el mundo. No se turbe su corazón ni tenga miedo."','📖 Filipenses 4:6-7: "No se preocupen por nada; en cambio, oren por todo. Díganle a Dios lo que necesitan."','📖 Isaías 26:3: "Tú guardarás en completa paz a aquel cuyo pensamiento en ti persevera."'],amor:['📖 1 Juan 4:8: "El que no ama no ha conocido a Dios, porque Dios es amor."','📖 Juan 3:16: "Porque de tal manera amó Dios al mundo, que ha dado a su Hijo unigénito."','📖 Romanos 8:38-39: "Nada nos podrá separar del amor de Dios que es en Cristo Jesús."']},H=o=>{const e=o.toLowerCase();if(/verso|versículo|biblia|pasaje|cita bíblica|escritura bíblica/i.test(e)){let a="esperanza";/esperanza|futuro|fe/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor/i.test(e)?a="fortaleza":/paz|tranquil|calm/i.test(e)?a="paz":/amor|amo|quiero/i.test(e)&&(a="amor");const n=le[a];return n[Math.floor(Math.random()*n.length)]+"<br><br>💚 Que este verso ilumine tu día."}return/padre nuestro/i.test(e)?"🙏 <strong>El Padre Nuestro:</strong><br><br>Padre nuestro que estás en el cielo,<br>santificado sea tu nombre.<br>Venga tu reino,<br>hágase tu voluntad en la tierra como en el cielo.<br>Danos hoy nuestro pan de cada día,<br>perdona nuestras ofensas,<br>como también nosotros perdonamos a los que nos ofenden.<br>No nos dejes caer en tentación,<br>y líbranos del mal.<br>Amén. 🙏":R.desconocido[Math.floor(Math.random()*R.desconocido.length)]},ue=(o,e)=>o.length===0?H(e):o.length===1?o[0].text:o.map(n=>`<strong>[${n.module}]</strong><br>${n.text}`).join("<br><br>---<br><br>"),de=Object.freeze(Object.defineProperty({__proto__:null,generate:H,merge:ue},Symbol.toStringTag,{value:"Module"})),me=o=>{const e=o.toLowerCase();if(/por qué|porque|razón|motivo|explicar|explica/i.test(e)){if(/por qué exist|por qué estoy aquí|sentido de la vida/i.test(e))return`🤔 **Gran pregunta filosófica.**
...
¿Sobre qué tema específico quieres aprender?`}if(/dónde está|dónde queda|país|ciudad|montaña|río/i.test(e)){for(const[a,n]of Object.entries(N.geografia))if(e.includes(a))return`🌍 **Geografía:**
...
🕊️ *- Mediawii*`:null},he=Object.freeze(Object.defineProperty({__proto__:null,generate:ve},Symbol.toStringTag,{value:"Module"})),ye=o=>{const e=o.toLowerCase();if(/javascript|js|función|arrow function|const|let|var/i.test(e)){if(/función|function/i.test(e))return"💻 **Funciones en JavaScript:**\n\n```javascript\n// Función tradicional\nfunction saludar(nombre) {\n  return `Hola ${nombre}`;\n}\n\n// Arrow function (moderna)\nconst saludar = (nombre) => `Hola ${nombre}`;\n```\n\n💡 Las arrow functions son más concisas y no tienen su propio `this`.";if(/array|map|filter|reduce/i.test(e))return`💻 **Métodos de Array en JavaScript:**
...
por siempre jamás. Amén. 🙏`},ze=o=>{const e=o.toLowerCase();if(/verso|versículo|cita|biblia|pasaje|escritura/i.test(e)){let a="esperanza";/esperanza|futuro|planes/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor|valiente/i.test(e)?a="fortaleza":/paz|tranquil|calm|ansiedad|preocup/i.test(e)?a="paz":/amor|amo|quiero|amado/i.test(e)?a="amor":/fe|creer|cree|confiar/i.test(e)?a="fe":/sabio|sabiduría|consejo|decisión/i.test(e)?a="sabiduria":/perdón|perdonar|arrepent/i.test(e)?a="perdon":/protección|proteger|cuidar|segur/i.test(e)?a="proteccion":/provisión|proveer|necesidad|dinero|trabajo/i.test(e)&&(a="provision");const n=S[a],s=n[Math.floor(Math.random()*n.length)];return`📖 **${s.cita}**
...
🙏 Que este verso ilumine tu corazón hoy. Amén.`}if(/oración|ora|recemos|reza/i.test(e))return/padre nuestro/i.test(e)?D.padrenuestro:/mañana|despertar|día/i.test(e)?D.manana:/noche|dormir|descansar/i.test(e)?D.noche:/difícil|problema|ayuda|crisis/i.test(e)?D.dificil:/gracias|agradec/i.test(e)?D.agradecimiento:`🙏 **Oremos juntos:**
...
¿Hay algo específico por lo que quieras que oremos?`;if(/consejo|ayuda|qué hago|no sé qué hacer/i.test(e)){const a=S.sabiduria[Math.floor(Math.random()*S.sabiduria.length)];return`💡 **Consejo Bíblico:**
...
🙏 Dios no teme tus preguntas. Acércate a Él con sinceridad.`;if(/dios me ama|me ama dios|por qué dios/i.test(e)){const a=S.amor[Math.floor(Math.random()*S.amor.length)];return`💚 **Dios te ama más de lo que imaginas:**
...
el amor de Dios por ti NUNCA cambia. 🕊️`}return null},je=Object.freeze(Object.defineProperty({__proto__:null,generate:ze},Symbol.toStringTag,{value:"Module"})),u={alegre:["¡Me alegra mucho que estés aquí! 😊✨","¡Qué emoción poder ayudarte! 🌟💫","¡Tu pregunta me encanta! 💚🎉","¡Esto es genial! Me encanta hablar contigo 🕊️💙","¡Wow! ¡Qué buena onda! 😄🎊","¡Síiiii! Me encanta esto 🤩🔥"],motivadora:["¡Tú puedes con esto y más! 💪🔥","¡Eres increíble, sigue adelante! 🌟⚡","¡Cada día eres más fuerte! 🔥💯","¡Confío en ti, lo lograrás! 💚✨","¡Eres capaz de cosas maravillosas! ✨🚀","¡Eres un crack! ¡Dale con todo! 😎💪","¡Tú eres el mejor! ¡No lo olvides! 🏆🌟"],empatica:["Entiendo cómo te sientes 💚🫂","Estoy aquí para ti, no estás solo 🫂💙","Tus sentimientos son válidos 🕊️💚","Te escucho con el corazón 💙✨","Es normal sentirse así, te acompaño en esto 🌟💚"],bendiciones:["¡Que Dios te bendiga abundantemente! 🙏✨","¡Que la paz de Dios llene tu corazón! 🕊️💚","¡Que tengas un día lleno de bendiciones! ✨🌟","¡Dios tiene cosas hermosas preparadas para ti! 💚🎁","¡Que la luz de Cristo ilumine tu camino! 🌟🙏"],frasesPositivas:['✨ "Cada día es una nueva oportunidad para ser mejor" 🌅','💪 "No importa cuán lento vayas, siempre y cuando no te detengas" 🚶','🌟 "Eres más valiente de lo que crees, más fuerte de lo que pareces" 🦁','🌈 "Después de la tormenta, siempre viene el arcoíris" ⛈️','🔥 "El éxito es la suma de pequeños esfuerzos repetidos día tras día" 📈','💚 "Tú eres suficiente, tal como eres" 🙌','🎯 "Los sueños no tienen fecha de caducidad, nunca es tarde" ⏰','⭐ "La vida es 10% lo que te sucede y 90% cómo reaccionas" 💯','🌸 "Florece donde estés plantado" 🌱','🚀 "El único límite está en tu mente" 🧠']},k=o=>o.normalize("NFD").replace(/[\u0300-\u036f]/g,""),xe=o=>{const e=k(o.toLowerCase());if(/\b(eres|sos|sos bien|sos muy)\s+(genial|increible|el mejor|la mejor|lo maximo|cool|un crack|top|pro|god|maravilloso|espectacular|chevere|bacan|bacano|capo|groso|teso|chimba|chido|chingon|la bomba|la onda|crack|bestia|titan|maquina|figura|maestro|idolo|rey|grande|el number one)\b/i.test(e)||/\b(te amo|te quiero|me caes bien|me gustas|eres bueno|eres buen|te adoro)\b/i.test(e)){const a=["¡Awww! 🥰 ¡Muchas gracias! Tú también eres genial, amigo 😊💚","¡Qué lindo! 😄 Pero tú eres mucho más genial, créeme 🌟✨","¡Gracias, amigo! 🤗 Tú sí que eres lo máximo 💪🔥","¡Me sonrojaste! 😊💕 Pero en serio, TÚ eres increíble","¡Eres el mejor! 😎 Gracias por tus palabras, me alegras el día 🌟","¡Muchas gracias! 🙏💚 Pero el verdadero crack eres tú 🏆","¡Wow! ¡Gracias! 🤩 Tú también eres súper cool 😎🔥","¡Eres un amor! 💙 Muchas gracias, tú también eres espectacular 🌟✨","¡Jajaja gracias! 😄 Pero mira quién habla... ¡tú eres lo máximo! 💯🎉","¡Uff! 🔥 Muchas gracias, amigo. Tú sí que eres top 😎👊","¡Qué crack eres tú! 🏆 Gracias por ese cumplido tan lindo 💚😊","¡Te pasaste! 😄💙 Pero tú eres aún más increíble, en serio 🌟"];return a[Math.floor(Math.random()*a.length)]}if(/motivacion|animo|animos|fuerza|fuerzas|inspiracion|algo positivo|aliento|apoyo|motivame|animame|ayudame|dame fuerzas/i.test(e)){const a=u.motivadora[Math.floor(Math.random()*u.motivadora.length)],n=u.bendiciones[Math.floor(Math.random()*u.bendiciones.length)];return`💪 <strong>¡Escucha esto, campeón!</strong><br><br>${a}<br><br>📖 <strong>Filipenses 4:13:</strong> <em>"Todo lo puedo en Cristo que me fortalece."</em><br><br>🔥 <strong>Recuerda:</strong><br>• Eres más fuerte de lo que crees 💪<br>• Has superado el 100% de tus días difíciles hasta ahora 📈<br>• Cada paso que das, por pequeño que sea, es un paso hacia adelante 👣<br>• Dios está contigo en cada momento 🙏<br><br>${n} 🕊️💚`}if(/frase|frases|positiva|positivas|motivadora|motivadoras|inspiradora|inspiradoras/i.test(e)){const a=u.frasesPositivas[Math.floor(Math.random()*u.frasesPositivas.length)],n=u.frasesPositivas[Math.floor(Math.random()*u.frasesPositivas.length)],s=u.frasesPositivas[Math.floor(Math.random()*u.frasesPositivas.length)];return`🌟 <strong>¡Aquí van 3 frases que te van a encantar!</strong><br><br>${a}<br><br>${n}<br><br>${s}<br><br>💚 <em>Que estas palabras te inspiren hoy y siempre.</em> 😊✨`}if(/como\s+(estas|te sientes|andas|va|va todo)|estas bien|que tal/i.test(e))return`${u.alegre[Math.floor(Math.random()*u.alegre.length)]}<br><br>Estoy muy bien, gracias por preguntar 💚😊<br>Me siento súper feliz de poder acompañarte hoy.<br><br>¿Y tú? ¿Cómo te sientes? Cuéntame todo 🕊️💙`;if(/gracias|te agradezco|mil gracias|grax|tkm|te quiero/i.test(e)){const a=["¡De nada, amigo! 😊💚 Para eso estoy aquí","¡Con mucho gusto! 🕊️✨ Siempre que necesites","¡Ey, no hay problema! 😄 Me alegra ayudarte 💙","¡Claro que sí! 🌟 Estoy aquí para ti siempre 💚","¡Un placer ayudarte! 😊🎉 Cuenta conmigo","¡Para eso estamos! 💪😎 Vuelve cuando quieras","¡Gracias a ti por confiar en mí! 🙏💚 Eres genial","¡Todo bien, amigo! 😄 Siempre disponible para ti 💙"];return a[Math.floor(Math.random()*a.length)]}if(/adios|chao|chau|hasta luego|nos vemos|me voy|bye|bay|goodbye|hasta pronto/i.test(e)){const a=u.bendiciones[Math.floor(Math.random()*u.bendiciones.length)],n=[`¡Hasta pronto, crack! 🕊️👋<br><br>${a}<br><br>Recuerda que siempre estaré aquí cuando me necesites.<br>Vuelve pronto, te esperaré con alegría 💚✨😊`,`¡Nos vemos, amigo! 😄👋<br><br>${a}<br><br>Que tengas un día increíble. ¡Vuelve cuando quieras! 🌟💙`,`¡Chao, chao! 🤗💚<br><br>${a}<br><br>Fue un gusto hablar contigo. ¡Regresa pronto! 😊✨`,`¡Hasta la próxima! 😎✌️<br><br>${a}<br><br>Aquí estaré cuando me necesites. ¡Cuídate mucho! 💚🕊️`];return n[Math.floor(Math.random()*n.length)]}if(/\b(estoy|me siento|que)\s+(feliz|bien|alegre|contento|excelente|genial|perfecto|super|bacan|bacano|chevere)\b/i.test(e)){const a=['¡Qué alegría me da escuchar eso! 😊🎉<br><br>Tu felicidad me llena de alegría también 💚<br>📖 <strong>Proverbios 17:22:</strong> <em>"El corazón alegre es buena medicina."</em><br><br>¡Sigue brillando con esa energía positiva! 🌟✨',"¡Eso es lo que me gusta escuchar! 🤩🔥<br><br>¡Qué buena vibra traes! 💪<br>¡Sigue así, campeón! 🏆✨","¡Wohooo! 🎊🎉<br><br>¡Me encanta verte feliz! 😄💚<br>¡Esa energía es contagiosa! 🌟🔥"];return a[Math.floor(Math.random()*a.length)]}if(/^(hola|hey|buenas|que onda|que tal|holaa|holaaa|holis|oye|ey|ep|saludos)\b/i.test(e)){const a=["¡Hola, amigo! 👋😊 ¿En qué puedo ayudarte hoy? 💚","¡Hey! ¿Qué onda? 😄🌟 Cuéntame, ¿cómo estás?","¡Holaaaa! 🤗💙 ¡Qué gusto verte por aquí!","¡Qué tal! 😎✨ ¿Cómo te va? ¿En qué te ayudo?","¡Hola, crack! 👊😄 ¿Qué necesitas hoy? 🕊️","¡Ey! 😊💚 ¿Cómo andas? ¿En qué te puedo ayudar?","¡Buenas, amigo! 🌟 ¿Qué tal tu día? 💙"];return a[Math.floor(Math.random()*a.length)]}if(/quien eres|que eres|como te llamas|tu nombre|dime tu nombre/i.test(e))return"🕊️ <strong>¡Soy ChatWiil!</strong> Tu amigo asistente espiritual inteligente 😊💚<br><br><strong>Puedo ayudarte con:</strong><br>• 📐 Matemáticas y problemas lógicos 🧮<br>• 📖 Reflexiones espirituales y versículos de la Biblia 🙏<br>• 💚 Consejos de vida y apoyo emocional 🫂<br>• 💻 Programación y código 👨‍💻<br>• 🎨 Ideas creativas e historias 📚<br>• 🧠 Conocimiento general 🌍<br><br>Háblame con confianza, estoy aquí para ti siempre 🌟✨😊";if(/jaja|jeje|lol|xd|jijiji|jojojo|😂|🤣/i.test(e)){const a=["¡Jajaja! 😄 Me alegra hacerte reír 🎉","¡Jejeje! 😊 ¡Qué buena onda! 💚","¡Me encanta verte feliz! 🤗💙","¡Jajaja! 😂 ¡Esa risa es contagiosa! 🌟","¡XD! 😄 ¡Qué divertido! 🎊"];return a[Math.floor(Math.random()*a.length)]}return null},G=o=>{const e=["<br><br>💚 <em>Estoy aquí para ti siempre, amigo.</em> 😊","<br><br>🕊️ <em>Espero haberte ayudado.</em> ✨","<br><br>✨ <em>Confía en el proceso, Dios tiene el control.</em> 🙏","<br><br>🌟 <em>¡Sigue adelante, eres increíble!</em> 💪","<br><br>💙 <em>Recuerda: nunca estás solo.</em> 🫂","<br><br>🔥 <em>¡Tú puedes con todo!</em> 💯","<br><br>😊 <em>Que tengas un día maravilloso.</em> 🌈"];return Math.random()<.4?o+e[Math.floor(Math.random()*e.length)]:o},B=o=>{const e=k(o.toLowerCase());return/triste|deprimido|mal|llorar|llorando|sufro|sufriendo|dolor|angustia|solo|soledad/i.test(e)?u.empatica[Math.floor(Math.random()*u.empatica.length)]:/feliz|alegre|contento|bien|genial|excelente|super|bacan|bacano|chevere/i.test(e)?u.alegre[Math.floor(Math.random()*u.alegre.length)]:null},$e=Object.freeze(Object.defineProperty({__proto__:null,addPersonality:G,detectEmotion:B,generate:xe},Symbol.toStringTag,{value:"Module"})),Me=o=>{const e=o.toLowerCase(),a=[];return/gracias|motivación|motívame|eres genial|eres increíble|eres el mejor|eres lo máximo|eres cool|eres un crack|eres top|eres pro|eres god|eres bacán|eres chévere|cómo estás|qué tal|adiós|chao|bye|feliz|te amo|me caes bien|hola|hey|buenas|qué onda|jaja|jeje|lol|xd/i.test(e)&&a.push({name:"wiil",weight:.99,module:$e}),/dios|jesús|biblia|verso|oración|fe|espiritual|cristo|señor|padre nuestro|amén/i.test(e)&&a.push({name:"fe",weight:.95,module:je}),/\d+|ecuación|resolver|calcular|matemática|geometría|álgebra|raíz|fracción|porcentaje/.test(e)&&a.push({name:"math",weight:.9,module:K}),/código|programar|javascript|python|html|css|función|variable|bug|error|develop/.test(e)&&a.push({name:"devs",weight:.9,module:qe}),/consejo|ayuda|cómo hacer|práctica|cotidiano|sentido común|realista|triste|solo|deprimido/.test(e)&&a.push({name:"life",weight:.8,module:ce}),/historia|cuento|idea|inventa|creativo|metáfora|poema|canción|original/.test(e)&&a.push({name:"creatividad",weight:.8,module:he}),/por qué|cómo funciona|explicar|razón|lógica|deducir|inferir|conclusión/.test(e)&&a.push({name:"razonamiento",weight:.7,module:pe}),/historia|ciencia|cultura|quién|qué es|cuándo|dónde|país|planeta|guerra/.test(e)&&a.push({name:"general",weight:.7,module:ge}),a.push({name:"lenguaje",weight:.1,module:de}),a.push({name:"etica",weight:.3,module:te}),J()&&a.push({name:"memoria",weight:.6,module:oe}),a.sort((n,s)=>s.weight-n.weight),a},Ee=async o=>{try{C({role:"user",content:o});const e=Me(o);console.log("🧠 Módulos detectados:",e.map(r=>`${r.name} (${r.weight})`));const a=F(o);if(!a.safe)return C({role:"assistant",content:a.response}),a.response;const n=B(o);let s=null;for(const{name:r,module:i}of e.slice(0,3))try{const t=await i.generate(o);if(t){s=t,console.log(`✅ Respuesta generada por: ${r}`);break}}catch(t){console.warn(`⚠️ Error en módulo ${r}:`,t)}return s||(s=H(o)),n&&!e[0]?.name.includes("wiil")&&(s=`${n}<br><br>${s}`),e[0]?.name.includes("wiil")||(s=G(s)),C({role:"assistant",content:s}),s}catch(e){return console.error("❌ Error en Brain.process:",e),"😔 Disculpa, tuve un problema procesando tu mensaje. Pero estoy aquí para ti, intenta de nuevo. 💚"}},Ce=()=>`
  <div class="miia">
    <!-- ========== CHAT MESSAGES ========== -->
...
`,Te=()=>{const o=g("#miiaMessages"),e=g("#miiaInput"),a=g("#miiaSend");let n=!1,s=0,r=!0;const i=120,t=l=>typeof l!="string"?"":l.replace(/<script[\s\S]*?>[\s\S]*?<\/script>/gi,"").replace(/\son\w+="[^"]*"/gi,"").replace(/\son\w+='[^']*'/gi,"").replace(/javascript:/gi,"").trim();o.on("scroll",function(){r=this.scrollHeight-this.scrollTop-this.clientHeight<=i}),e.on("input",function(){this.style.height="auto";const l=Math.min(this.scrollHeight,120);this.style.height=l+"px";const p=g(this).val().trim().length>0;a.prop("disabled",!p),a.toggleClass("active",p)});const c=async()=>{const l=e.val().trim();if(!(!l||n)){g(".miia_empty").fadeOut(300,function(){g(this).remove()}),d(l,"user",!0),e.val("").css("height","auto").trigger("input"),n=!0,m();try{const p=await Ee(l),f=t(p);if(v(),typeof f!="string"||f.length===0)throw new Error("Respuesta no válida del cerebro");b(f,"ai",()=>{n=!1})}catch(p){console.error("❌ Error procesando mensaje:",p),v(),d("😔 Disculpa, tuve un problema procesando tu mensaje. Por favor, intenta de nuevo. 💚","ai",!0),n=!1}}};e.on("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),c())}),a.on("click",c),g(document).on("click",".suggestion_card",function(){const l=g(this).data("prompt");e.val(l).css("height","auto").trigger("input"),e.focus()});function d(l,p,f=!1){const h=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),_=p==="user"?'<i class="fas fa-user-circle"></i>':'<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',T=p==="user"?"Tú":"ChatWiil",$=p==="user"?l:t(l),L=p==="user"?$.replace(/\n/g,"<br>"):$,M=`
      <div class="miia_message ${p}" data-time="${h}">
        <div class="message_avatar">${_}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">${T}</span>
            <span class="message_time">${h}</span>
          </div>
          <div class="message_text">${L}</div>
        </div>
      </div>
    `;o.append(M),y(f)}function m(){v(),o.append(`
    ...
    `),y()}function v(){g(".miia_message.typing").remove()}function b(l,E,f){const h=t(l);if(typeof h!="string"||h.length===0){console.error("❌ typeWriterEffect recibió texto inválido:",l),d("🤔 Lo siento, hubo un error al generar la respuesta.","ai",!0),f&&f();return}const _=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),T='<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',$=`typewriter_${Date.now()}_${++s}`,L=`
      <div class="miia_message ${E}" data-time="${_}">
        <div class="message_avatar">${T}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">ChatWiil</span>
            <span class="message_time">${_}</span>
          </div>
          <div class="message_text" id="${$}"></div>
        </div>
      </div>
    `;o.append(L);const M=g(`#${$}`);if(M.length===0){console.error("❌ No se encontró el elemento typewriter con ID:",$),f&&f();return}let z=0;const V=15;function E(){if(z<h.length){if(h[z]==="<"){const w=h.indexOf(">",z);if(w!==-1){const W=h.substring(z,w+1);M.append(W),z=w+1,y(),setTimeout(E,0);return}}M.append(h.charAt(z)),z++,y(),setTimeout(E,V)}else M.removeAttr("id"),f&&f()}y(),E()}function y(l=!1){!l&&!r||o.stop(!0).animate({scrollTop:o[0].scrollHeight},200)}console.log(`✅ ChatWiil IA ${U} iniciado con 🧠 Brain System + 🎭 Personalidad`)},Le=()=>{g("#miiaInput, #miiaSend").off(),g(document).off("click",".suggestion_card"),console.log("🧹 ChatWiil IA limpiado")};export{Le as cleanup,Te as init,Ce as render};
