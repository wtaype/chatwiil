...
`,Ee=()=>{const o=f("#miiaMessages"),e=f("#miiaInput"),a=f("#miiaSend");let n=!1;e.on("input",function(){this.style.height="auto";const i=Math.min(this.scrollHeight,120);this.style.height=i+"px";const u=f(this).val().trim().length>0;a.prop("disabled",!u),a.toggleClass("active",u)});const s=async()=>{const i=e.val().trim();if(!(!i||n)){f(".miia_empty").fadeOut(300,function(){f(this).remove()}),r(i,"user"),e.val("").css("height","auto").trigger("input"),n=!0,c();try{const u=await qe(i);t(),l(typeof u=="string"?u:"🤔 Lo siento, hubo un error al procesar tu mensaje.","ai",()=>{n=!1})}catch(u){console.error("Error en brain.process:",u),t(),r("🤔 Lo siento, hubo un error al procesar tu mensaje. Por favor, intenta de nuevo.","ai"),n=!1}}};e.on("keydown",i=>{i.key==="Enter"&&!i.shiftKey&&(i.preventDefault(),s())}),a.on("click",s),f(document).on("click",".suggestion_card",function(){const i=f(this).data("prompt");e.val(i).css("height","auto").trigger("input"),e.focus()});function r(i,u){const p=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),g=u==="user"?'<i class="fas fa-user-circle"></i>':'<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',C=u==="user"?"Tú":"ChatWiil",E=u==="user"?i.replace(/\n/g,"<br>"):i,q=`
      <div class="miia_message ${u}" data-time="${p}">
        <div class="message_avatar">${g}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">${C}</span>
            <span class="message_time">${p}</span>
          </div>
          <div class="message_text">${E}</div>
        </div>
      </div>
    `;o.append(q),d()}function c(){o.append(`
    ...
    `),d()}function t(){f(".miia_message.typing").remove()}function l(i,z,p){const g=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),E=`
