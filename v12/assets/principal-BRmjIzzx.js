import{j as g}from"./vendor-gzd0YkcT.js";import{v as V}from"./wii-Ki-EYAEo.js";import"./main-4VzfTA_K.js";import"./main-3cVTOUyD.js";const S=["¡Genial! 🎯 Las matemáticas son divertidas","¡Excelente pregunta! 🌟 Aquí va la respuesta","¡Me encanta resolver esto! 🚀 Vamos allá","¡Qué emoción! 💡 Resolvamos esto juntos","¡Perfecto! 🎨 Las mates son arte","¡Súper! ⭐ Te va a encantar esto"],L=["¡Boom! 💥 Ahí está la respuesta","¡Tachán! ✨ Resuelto","¡Voilà! 🎭 Problema solucionado","¡Eureka! 🎉 Lo logramos","¡Bingo! 🎯 Ahí lo tienes","¡Fantástico! 🌟 Resultado final"],q=o=>o[Math.floor(Math.random()*o.length)],W=o=>{const e=o.toLowerCase(),a=e.match(/(\d+)x\s*([+-])\s*(\d+)\s*=\s*(\d+)/);if(a){const[s,r,c,t,l]=a,d=parseInt(r),p=c==="+"?parseInt(t):-parseInt(t),m=parseInt(l)-p,i=m/d;return`${q(S)} 😊<br><br>📊 <strong>Resolución paso a paso:</strong><br><br><strong>Ecuación:</strong> ${r}x ${c} ${t} = ${l}<br><br><strong>Paso 1:</strong> Despejar x moviendo el ${Math.abs(p)} 🔄<br>${r}x = ${l} ${c==="+"?"-":"+"} ${Math.abs(p)}<br>${r}x = ${m}<br><br><strong>Paso 2:</strong> Dividir ambos lados entre ${r} ➗<br>x = ${m} ÷ ${r}<br><br>${q(L)} 🎊<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ x = ${i}</span><br><br><em>🔍 Verificación: ${r}(${i}) ${c} ${t} = ${d*i+p} = ${l} ✓</em> 😄`}if(/porcentaje|%/.test(e)){const s=e.match(/(\d+)%\s*de\s*(\d+)/);if(s){const[r,c,t]=s,l=parseInt(c)*parseInt(t)/100;return`${q(S)} 📈<br><br>📊 <strong>Cálculo de porcentaje:</strong><br><br>${c}% de ${t} = (${c} × ${t}) ÷ 100 🧮<br>= ${parseInt(c)*parseInt(t)} ÷ 100<br><br>${q(L)} 🎉<br><span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">✅ Resultado: ${l}</span> 😊`}}const n=e.match(/(\d+)\s*([+\-*/])\s*(\d+)/);if(n){const[s,r,c,t]=n,l=parseFloat(r),d=parseFloat(t);let p,b,m;switch(c){case"+":p=l+d,b="+",m="➕";break;case"-":p=l-d,b="-",m="➖";break;case"*":p=l*d,b="×",m="✖️";break;case"/":p=l/d,b="÷",m="➗";break}return`${q(S)} ${m}<br><br>🔢 <strong>Operación:</strong><br><br>${r} ${b} ${t} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${p}</span> ✅ 😄`}if(/área|perímetro|cuadrado|rectángulo|círculo/.test(e)){const s=e.match(/cuadrado.*?(\d+)/);if(s){const r=parseInt(s[1]);return`¡Me encantan las figuras geométricas! 📐😊<br><br><strong>Cuadrado de lado ${r}:</strong><br><br>• <strong>Área</strong> = lado² = ${r}² = <span style="color: #2196f3; font-weight: bold;">${r*r}</span> unidades² 🟦<br>• <strong>Perímetro</strong> = 4 × lado = 4 × ${r} = <span style="color: #2196f3; font-weight: bold;">${4*r}</span> unidades 📏<br><br>¡Facilísimo! 🎉😄`}}if(/raíz cuadrada|√/.test(e)){const s=e.match(/raíz cuadrada de (\d+)|√(\d+)/);if(s){const r=parseInt(s[1]||s[2]),c=Math.sqrt(r),t=Number.isInteger(c);return`${q(S)} 🌱<br><br>🔢 <strong>Raíz cuadrada:</strong><br><br>√${r} = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${c.toFixed(2)}</span> ${t?"✅ ¡Exacto!":"<em>(aproximado)</em>"} 😊`}}if(/elevado|potencia|\^/.test(e)){const s=e.match(/(\d+)\s*(?:elevado a|potencia|\^)\s*(\d+)/);if(s){const[r,c,t]=s,l=Math.pow(parseInt(c),parseInt(t));return`¡Las potencias son poderosas! 💪⚡<br><br>🔢 <strong>Potencia:</strong><br><br>${c}<sup>${t}</sup> = <span style="color: #4caf50; font-size: 1.3em; font-weight: bold;">${l}</span> ✅<br><br>${q(L)} 🎊😄`}}if(/tabla de multiplicar|tabla del/i.test(e)){const s=e.match(/tabla del? (\d+)/i);if(s){const r=parseInt(s[1]);let c=`¡La tabla del ${r}! 📚 ¡Vamos a aprenderla! 😊<br><br>`;c+=`🔢 <strong>Tabla de multiplicar del ${r}:</strong><br><br>`;for(let t=1;t<=10;t++)c+=`${r} × ${t} = <strong style="color: #2196f3;">${r*t}</strong> ${t===5?"🌟":""}<br>`;return c+="<br>¡Practica y serás un experto! 🚀😄",c}}if(/primo|primos/.test(e)){const s=e.match(/(\d+)\s+es primo/);if(s){const r=parseInt(s[1]),t=(l=>{if(l<=1)return!1;if(l<=3)return!0;if(l%2===0||l%3===0)return!1;for(let d=5;d*d<=l;d+=6)if(l%d===0||l%(d+2)===0)return!1;return!0})(r);return`🔍 Analizando si ${r} es primo...<br><br>${t?`¡Sí! 🎉 <strong style="color: #4caf50;">${r} ES un número primo</strong> 😊<br><br>Solo es divisible por 1 y por sí mismo 🌟`:`No 😔 <strong style="color: #f44336;">${r} NO es primo</strong><br><br>Es divisible por otros números además de 1 y ${r} 🔢`}`}}return null},U={formulas:{cuadrado:{area:"lado²",perimetro:"4 × lado"},rectangulo:{area:"base × altura",perimetro:"2(base + altura)"},circulo:{area:"π × radio²",perimetro:"2π × radio"},triangulo:{area:"(base × altura) / 2"}},conceptos:{ecuacion:"Igualdad matemática con incógnitas",fraccion:"Parte de un todo (numerador/denominador)",porcentaje:"Fracción de 100 (x/100)",primo:"Número solo divisible por 1 y por sí mismo"}},Y=Object.freeze(Object.defineProperty({__proto__:null,generate:W,knowledge:U},Symbol.toStringTag,{value:"Module"}));let h=[];const N=20,_=o=>{h.push({...o,timestamp:Date.now()}),h.length>N&&(h=h.slice(-N))},I=(o=5)=>h.slice(-o),R=()=>h.length>0,X=o=>h.filter(e=>e.content.toLowerCase().includes(o.toLowerCase())),K=()=>{h=[]},Z=o=>{const e=o.toLowerCase(),a=I(5);if(/anterior|antes|dijiste|mencionaste|lo que|eso|aquello/.test(e)){const n=[...a].reverse().find(s=>s.role==="assistant");if(n)return`🧠 **Recordando la conversación...**
...
Estoy aquí si necesitas hablar más. 💚`}return/solo|soledad|nadie|aislado/.test(e)?`${z.soledad[Math.floor(Math.random()*z.soledad.length)]}
...
Recuerda: **La soledad es temporal. Las conexiones empiezan con un paso.** 🫂`:/motivación|motivado|ganas|ánimo|energía/.test(e)?`${z.motivacion[Math.floor(Math.random()*z.motivacion.length)]}
...
`+z.mejora.slice(0,3).map((n,s)=>`${s+1}. ${n}`).join(`
...
Háblame con confianza, te escucho. 🕊️`:null},te=Object.freeze(Object.defineProperty({__proto__:null,generate:re},Symbol.toStringTag,{value:"Module"})),O={desconocido:["🤔 Hmm, no estoy seguro de cómo responder a eso. ¿Podrías reformular tu pregunta?","💭 Interesante pregunta. ¿Podrías darme más detalles para ayudarte mejor?","🕊️ No tengo una respuesta clara para eso, pero intentaré ayudarte. ¿Puedes ser más específico?"]},ie={esperanza:['📖 Jeremías 29:11: "Yo sé los planes que tengo para ustedes, planes de bienestar y no de calamidad, a fin de darles un futuro y una esperanza."','📖 Romanos 15:13: "Que el Dios de la esperanza los llene de toda alegría y paz en la fe, para que rebosen de esperanza por el poder del Espíritu Santo."','📖 Salmo 33:18: "Los ojos del Señor están sobre los que le temen, sobre los que esperan en su gran amor."'],fortaleza:['📖 Isaías 41:10: "No temas, porque yo estoy contigo; no desmayes, porque yo soy tu Dios que te fortalezco."','📖 Filipenses 4:13: "Todo lo puedo en Cristo que me fortalece."','📖 Josué 1:9: "Sé fuerte y valiente. No temas ni desmayes, porque el Señor tu Dios estará contigo dondequiera que vayas."'],paz:['📖 Juan 14:27: "La paz les dejo, mi paz les doy. No se la doy como la da el mundo. No se turbe su corazón ni tenga miedo."','📖 Filipenses 4:6-7: "No se preocupen por nada; en cambio, oren por todo. Díganle a Dios lo que necesitan."','📖 Isaías 26:3: "Tú guardarás en completa paz a aquel cuyo pensamiento en ti persevera."'],amor:['📖 1 Juan 4:8: "El que no ama no ha conocido a Dios, porque Dios es amor."','📖 Juan 3:16: "Porque de tal manera amó Dios al mundo, que ha dado a su Hijo unigénito."','📖 Romanos 8:38-39: "Nada nos podrá separar del amor de Dios que es en Cristo Jesús."']},A=o=>{const e=o.toLowerCase();if(/verso|versículo|biblia|pasaje|cita bíblica|escritura bíblica/i.test(e)){let a="esperanza";/esperanza|futuro|fe/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor/i.test(e)?a="fortaleza":/paz|tranquil|calm/i.test(e)?a="paz":/amor|amo|quiero/i.test(e)&&(a="amor");const n=ie[a];return n[Math.floor(Math.random()*n.length)]+"<br><br>💚 Que este verso ilumine tu día."}return/padre nuestro/i.test(e)?"🙏 <strong>El Padre Nuestro:</strong><br><br>Padre nuestro que estás en el cielo,<br>santificado sea tu nombre.<br>Venga tu reino,<br>hágase tu voluntad en la tierra como en el cielo.<br>Danos hoy nuestro pan de cada día,<br>perdona nuestras ofensas,<br>como también nosotros perdonamos a los que nos ofenden.<br>No nos dejes caer en tentación,<br>y líbranos del mal.<br>Amén. 🙏":O.desconocido[Math.floor(Math.random()*O.desconocido.length)]},ce=(o,e)=>o.length===0?A(e):o.length===1?o[0].text:o.map(n=>`<strong>[${n.module}]</strong><br>${n.text}`).join("<br><br>---<br><br>"),le=Object.freeze(Object.defineProperty({__proto__:null,generate:A,merge:ce},Symbol.toStringTag,{value:"Module"})),ue=o=>{const e=o.toLowerCase();if(/por qué|porque|razón|motivo|explicar|explica/i.test(e)){if(/por qué exist|por qué estoy aquí|sentido de la vida/i.test(e))return`🤔 **Gran pregunta filosófica.**
...
¿Sobre qué tema específico quieres aprender?`}if(/dónde está|dónde queda|país|ciudad|montaña|río/i.test(e)){for(const[a,n]of Object.entries(w.geografia))if(e.includes(a))return`🌍 **Geografía:**
...
🕊️ *- Mediawii*`:null},be=Object.freeze(Object.defineProperty({__proto__:null,generate:ge},Symbol.toStringTag,{value:"Module"})),ve=o=>{const e=o.toLowerCase();if(/javascript|js|función|arrow function|const|let|var/i.test(e)){if(/función|function/i.test(e))return"💻 **Funciones en JavaScript:**\n\n```javascript\n// Función tradicional\nfunction saludar(nombre) {\n  return `Hola ${nombre}`;\n}\n\n// Arrow function (moderna)\nconst saludar = (nombre) => `Hola ${nombre}`;\n```\n\n💡 Las arrow functions son más concisas y no tienen su propio `this`.";if(/array|map|filter|reduce/i.test(e))return`💻 **Métodos de Array en JavaScript:**
...
por siempre jamás. Amén. 🙏`},ye=o=>{const e=o.toLowerCase();if(/verso|versículo|cita|biblia|pasaje|escritura/i.test(e)){let a="esperanza";/esperanza|futuro|planes/i.test(e)?a="esperanza":/fuerza|fuerte|fortaleza|valor|valiente/i.test(e)?a="fortaleza":/paz|tranquil|calm|ansiedad|preocup/i.test(e)?a="paz":/amor|amo|quiero|amado/i.test(e)?a="amor":/fe|creer|cree|confiar/i.test(e)?a="fe":/sabio|sabiduría|consejo|decisión/i.test(e)?a="sabiduria":/perdón|perdonar|arrepent/i.test(e)?a="perdon":/protección|proteger|cuidar|segur/i.test(e)?a="proteccion":/provisión|proveer|necesidad|dinero|trabajo/i.test(e)&&(a="provision");const n=E[a],s=n[Math.floor(Math.random()*n.length)];return`📖 **${s.cita}**
...
`,Pe=()=>{const o=g("#miiaMessages"),e=g("#miiaInput"),a=g("#miiaSend");let n=!1,s=0,r=!0;const c=120;o.on("scroll",function(){r=this.scrollHeight-this.scrollTop-this.clientHeight<=c}),e.on("input",function(){this.style.height="auto";const i=Math.min(this.scrollHeight,120);this.style.height=i+"px";const f=g(this).val().trim().length>0;a.prop("disabled",!f),a.toggleClass("active",f)});const t=async()=>{const i=e.val().trim();if(!(!i||n)){g(".miia_empty").fadeOut(300,function(){g(this).remove()}),l(i,"user",!0),e.val("").css("height","auto").trigger("input"),n=!0,d();try{const f=await Me(i);if(p(),typeof f!="string")throw new Error("Respuesta no válida del cerebro");b(f,"ai",()=>{n=!1})}catch(f){console.error("❌ Error procesando mensaje:",f),p(),l("😔 Disculpa, tuve un problema procesando tu mensaje. Por favor, intenta de nuevo. 💚","ai",!0),n=!1}}};e.on("keydown",i=>{i.key==="Enter"&&!i.shiftKey&&(i.preventDefault(),t())}),a.on("click",t),g(document).on("click",".suggestion_card",function(){const i=g(this).data("prompt");e.val(i).css("height","auto").trigger("input"),e.focus()});function l(i,f,v=!1){const $=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),P=f==="user"?'<i class="fas fa-user-circle"></i>':'<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',M=f==="user"?"Tú":"ChatWiil",C=f==="user"?i.replace(/\n/g,"<br>"):i,j=`
      <div class="miia_message ${f}" data-time="${$}">
        <div class="message_avatar">${P}</div>
        <div class="message_content">
          <div class="message_header">
            <span class="message_name">${M}</span>
            <span class="message_time">${$}</span>
          </div>
          <div class="message_text">${C}</div>
        </div>
      </div>
    `;o.append(j),m(v)}function d(){p(),o.append(`
    ...
    `),m()}function p(){g(".miia_message.typing").remove()}function b(i,x,v){if(typeof i!="string"||i.length===0){console.error("❌ typeWriterEffect recibió texto inválido:",i),l("🤔 Lo siento, hubo un error al generar la respuesta.","ai",!0),v&&v();return}const $=new Date().toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"}),P='<img src="/smile.avif" alt="ChatWiil" class="message_avatar_img">',M=`typewriter_${Date.now()}_${++s}`,C=`
      <div class="miia_message ${x}" data-time="${$}">
        <div class="message_avatar">${P}</div>
        <div class="message_content">
          <div class="message_header">
    ...
    `;o.append(C);const j=g(`#${M}`);if(j.length===0){console.error("❌ No se encontró el elemento typewriter con ID:",M),v&&v();return}let y=0;const G=15;function x(){if(y<i.length){if(i[y]==="<"){const T=i.indexOf(">",y);if(T!==-1){const B=i.substring(y,T+1);j.append(B),y=T+1,m(),setTimeout(x,0);return}}j.append(i.charAt(y)),y++,m(),setTimeout(x,G)}else j.removeAttr("id"),v&&v()}m(),x()}function m(i=!1){!i&&!r||o.stop(!0).animate({scrollTop:o[0].scrollHeight},200)}console.log(`✅ ChatWiil IA ${V} iniciado con 🧠 Brain System + 🎭 Personalidad`)},Ce=()=>{g("#miiaInput, #miiaSend").off(),g(document).off("click",".suggestion_card"),console.log("🧹 ChatWiil IA limpiado")};export{Ce as cleanup,Pe as init,_e as render};
