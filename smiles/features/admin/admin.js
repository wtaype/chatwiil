...
// Jesús es mi Señor 🙏
// ════════════════════════════════════════════════════════════════════
import estilosAdmin from './admin.css?inline';
import { db } from '@core/firebase-db.js';
import { collection, query, where, getCountFromServer } from 'firebase/firestore';
import { savels, getls, Notificacion } from '@core/widev/widev.js';
import { app } from '@wii';
...
export const init = async () => {
  ...
  addDocListener('click', '#am_btn_refresh', function () {
...
function _cargarLocal() {
  const totEl = document.getElementById('am_n_total');
  const proEl = document.getElementById('am_n_pro');
  const orgEl = document.getElementById('am_n_org');
  const lesEl = document.getElementById('am_n_les');
  if (totEl) totEl.textContent = getls(K_TOT) || '—';
  if (proEl) proEl.textContent = getls(K_PRO) || '—';
  if (orgEl) orgEl.textContent = getls(K_ORG) || '—';
  if (lesEl) lesEl.textContent = getls(K_LES) || '45';
}
...
async function _cargarStats(forzar = false) {
  try {
    ...
    if (forzar || !getls(K_PRO)) {
      ...
      savels(K_PRO, val);
      const proEl = document.getElementById('am_n_pro');
      if (proEl) proEl.textContent = val;
    }
    ...
    if (forzar || !getls(K_ORG)) {
      ...
      savels(K_ORG, val);
      const orgEl = document.getElementById('am_n_org');
      if (orgEl) orgEl.textContent = val;
    }
